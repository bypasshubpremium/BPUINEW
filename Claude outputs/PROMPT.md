...
## Elements (all return an object with `.Value`, `:Set(value, silent)`, `:Get()`, `:SetVisible(bool)`, `:Destroy()`)
...
- The built-in Settings tab already provides Save / Load / Auto Load. Do not build your own config UI.
...
3. Loops for toggles must stop when the toggle is turned off: store the state in a variable and use `while state do task.wait() end` or disconnect connections.
