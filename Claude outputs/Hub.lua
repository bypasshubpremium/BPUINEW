--[[
    ...
    --------------------------------
    Loaded by Loader.lua when the current game has no dedicated script.
    The loader already put the library in getgenv().BPUI; the first lines
    below make this file also work when executed on its own.
]]
...
local Window = BPUI:CreateWindow({
    ...
    ConfirmClose = true,
    KeySystem = {
        Enabled = false,           -- set to true and fill in Keys / GetKeyLink to enable
